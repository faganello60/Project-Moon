...
# Bind system math library's Bessel function of the first kind (integer order)
# This avoids dependency on Scipy's Cython interface which can be tricky to link
libm_path = find_library('m') or find_library('System')
if not libm_path:
    # Fallback for some systems where find_library might fail, though unlikely on standard OS
    import sys
    if sys.platform == 'darwin':
        libm_path = 'libSystem.B.dylib'
    else:
        libm_path = 'libm.so.6'
...
try:
    ...
    _jn_func = libm.jn
    _jn_func.argtypes = [ctypes.c_int, ctypes.c_double]
    _jn_func.restype = ctypes.c_double
except (OSError, AttributeError):
    # If we really can't find it, we might need a pure python fallback or error out
    # For now, let's assume standard C math library is present
    raise RuntimeError("Could not load standard C math library 'jn' function.")

@njit
def njit_jv(n, x):
    """Numba-compatible wrapper for libc jn(n, x)"""
    return _jn_func(int(n), float(x))
...
@njit
def bespr(n, x):
    """Derivative of Bessel function J_n(x) with respect to x using recurrence relation."""
    # J'_n(x) = 0.5 * (J_{n-1}(x) - J_{n+1}(x))
    return 0.5 * (njit_jv(n - 1, x) - njit_jv(n + 1, x))

@njit
def sear(x_val):
    """Linear interpolation equivalent to Fortran SEAR subroutine."""
    # Accessing global numpy arrays XD and FD is supported in Numba if they are read-only
    if x_val <= XD[0]:
        return FD[0] 
    if x_val >= XD[-1]:
        return FD[-1]
...
@njit(parallel=True)
def _gyro_kernel(freq_arr, delta, nelectron, bmag, asize, viewangle, height, j1, j2, etr, np_dens):
    kf = len(freq_arr)
...
def gyro(freq, delta, nelectron, bmag, asize):
    ...
    Wrapper around _gyro_kernel for Numba compatibility.
    """
    ...
    # Call jitted kernel
    flux = _gyro_kernel(freq_arr, delta, nelectron, bmag, asize, viewangle, height, j1, j2, etr, np_dens)
